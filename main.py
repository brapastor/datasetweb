...
def _new_scraper(news_site_uid):
    ...
    logging.info('Begenning scraper for {}'.format(host))
...
def _fetch_article(news_site_uid, host, link):
    logger.info("Start fetching artiche at {}".format(link))

    article = None
    try:
        article = news.ArticlePage(news_site_uid, _build_link(host, link))
    except (HTTPError ,MaxRetryError) as e:
        logger.warning('Error while fectching the article', exc_info=False)
